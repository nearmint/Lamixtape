...
- **Statut** : Résolu Phase 6 (Axe C). Décision business utilisateur (Décision 5) : type `MusicPlaylist` retenu plutôt que `Article` — cohérence sémantique (Lamixtape EST une plateforme de playlists curatées) prime sur la couverture rich results Google qui est plus large pour `Article`. Possibilité future d'ajouter un schema `Article` en parallèle via `@graph` sans rien casser. Implémentation dans `inc/seo.php` (même fichier que OTHER-003) : nouvelle fonction `lmt_emit_jsonld_musicplaylist()` hookée sur `wp_head` priorité 20, avec early return si Rank Math actif (`defined('RANK_MATH_VERSION')`) pour éviter la double émission JSON-LD. Champs émis sur `is_singular('post')` : `@context` schema.org, `@type` MusicPlaylist, `name` (titre), `description` (excerpt 30 mots ou content fallback), `url` (permalink), `datePublished` ISO 8601, `dateModified` ISO 8601, `image` (post thumbnail si dispo), `author` (Person avec curator name), `numTracks` (count tracklist ACF repeater), `track[]` (MusicRecording avec `name` + `url` extraits via `get_field('tracklist')` directement de l'ACF — pas de placeholder fake si tracklist vide). `wp_json_encode` avec `JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE` pour JSON-LD valide.
