...
- **Statut** : Résolu Phase 5 (`9ba2f26` `title=` hardcodé dans `template-parts/card-mixtape.php`, paramètre `tag_link_attr` supprimé du template-part et de tous ses callers — `index.php`, `search.php`, `single.php`, `category.php`, `inc/rest.php` x3 contexts. Le paramètre n'avait plus qu'une seule valeur valide après le fix, sa suppression simplifie l'API et garantit l'invalidité HTML ne peut plus revenir) **+ Phase 8 cleanup ad-hoc** (oubli Phase 5 corrigé : `single.php:18` avait sa propre boucle de catégories indépendante du template-part `card-mixtape.php`, n'avait pas été migrée du `alt=` invalide vers `title=`. Le grep initial Phase 5 n'avait pas couvert tous les `*.php`. Fix appliqué dans le commit Phase 8 ad-hoc — cf. apprentissage **A11Y-PARTIAL** dans CLAUDE.md).
...
- **Statut** : Résolu Phase 6 (Axe C) **+ refactor post-Phase-7 ad-hoc** (1er mai 2026). **Phase 6** : décision business `MusicPlaylist` plutôt que `Article` (cohérence sémantique Lamixtape = playlists curatées). Implémentation initiale dans `inc/seo.php` : `lmt_emit_jsonld_musicplaylist()` hookée sur `wp_head` priorité 20 avec early return si Rank Math actif. **Refactor post-Phase-7 ad-hoc** : Phase 7 audit (`_docs/audit-post-refacto.md` section 2.3) a détecté que Rank Math n'émettait AUCUN JSON-LD sur les single mixtapes (module Schema RM désactivé sur post type `post`), et le fallback Phase 6 ne s'émettait pas non plus à cause de l'early return trop défensif. Refactor : extraction de `lmt_build_musicplaylist_data()` (helper DRY) + nouveau hook `add_filter( 'rank_math/json_ld', 'lmt_inject_musicplaylist_to_rank_math', 20, 1 )` qui injecte MusicPlaylist dans le `$data` array de RM (qui devient le `@graph` final). RM unifie l'émission dans son `<script class="rank-math-schema">` même si initialement vide pour singles. Le standalone fallback `lmt_emit_jsonld_musicplaylist()` (RM inactif) reste intact, partage le même helper `lmt_build_musicplaylist_data()`. **Vérification post-refactor** : single émet `<script class="rank-math-schema">{"@graph":[{"@type":"MusicPlaylist","name":"...","track":[...10 MusicRecording...]}]}</script>`. Home garde son @graph original (Organization/WebSite/WebPage/ImageObject/SearchAction) sans pollution MusicPlaylist (filter callback skip si `!is_singular('post')`). Commentaires `<!-- Phase 6 OTHER-006 fallback -->` absents (RM actif → fallback skipped, OK).
